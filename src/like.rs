...
pub async fn list(path: Path<(String,)>, pool: Data<DBPool>) -> HttpResponse {
    ...
    let likes =
        web::block(move || list_likes(Uuid::from_str(path.0.0.as_str()).unwrap(), &conn)).await;
...
pub async fn plus_one(path: Path<(String,)>, pool: Data<DBPool>) -> HttpResponse {
    ...
    let like =
        web::block(move || create_like(Uuid::from_str(path.0.0.as_str()).unwrap(), &conn)).await;
...
pub async fn minus_one(path: Path<(String,)>, pool: Data<DBPool>) -> HttpResponse {
    ...
    let _ = web::block(move || delete_like(Uuid::from_str(path.0.0.as_str()).unwrap(), &conn)).await;
